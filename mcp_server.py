...
def switch_model(model_name: str) -> str:
    """Switch models: 'hiyori' (Live2D), 'nagatoro_sprite' (sprite), 'succubus' (sprite). Nagatoro sprite: https://github.com/nukkerman2000/nagatoru-live2d"""
    if widget_ref is None:
...
@mcp.tool()
def play_succubus_clip(clip_name: str, loop: bool = True) -> str:
    """Play a succubus sprite animation clip (e.g. base1, base2, love1, love2). Check list_succubus_clips() for available clips. Loop=true to repeat indefinitely."""
    if widget_ref is None:
        return "Error: Widget not initialized"
    try:
        return widget_ref.play_succubus_clip(clip_name, loop)
    except Exception as e:
        return f"Error: {e}"


@mcp.tool()
def set_succ_speed(mult: float) -> str:
    """Set succubus playback speed multiplier. Integer values: -5,-4,-3,-2,-1 (reverse), 0 (pause), 2,3,4,5,6 (fast forward). 1 = normal speed."""
    if widget_ref is None:
        return "Error: Widget not initialized"
    try:
        return widget_ref.set_succ_speed(mult)
    except Exception as e:
        return f"Error: {e}"


@mcp.tool()
def set_succ_scale(delta_pct: float) -> str:
    """Resize the succubus character. delta_pct is a percent-of-screen-height change (positive = grow, negative = shrink). Clamped to 5% (min) and 80% (max) of screen height."""
    if widget_ref is None:
        return "Error: Widget not initialized"
    try:
        return widget_ref.set_succ_scale(delta_pct)
    except Exception as e:
        return f"Error: {e}"


@mcp.tool()
def stop_succubus_clip() -> str:
    """Stop the currently playing succubus sprite animation."""
    if widget_ref is None:
        return "Error: Widget not initialized"
    try:
        return widget_ref.stop_succubus_clip()
    except Exception as e:
        return f"Error: {e}"


@mcp.tool()
def list_succubus_clips() -> str:
    """List available succubus sprite animation clips."""
    from live2d_display import _load_succubus_clips
    if widget_ref is None:
        return "Error: Widget not initialized"
    try:
        if widget_ref._succubus_clips:
            clips = list(widget_ref._succubus_clips.keys())
        else:
            clips = list(_load_succubus_clips().keys())
        detail = {c: len(widget_ref._succubus_clips.get(c) or []) for c in clips}
        return json.dumps({"clips": clips, "frames": detail}, ensure_ascii=False)
    except Exception as e:
        return f"Error: {e}"
...
def get_status() -> str:
    ...
    if widget_ref:
        try:
            ...
            pass
    try:
        if widget_ref is not None and getattr(widget_ref, "_clip_current", None):
            info["succubus_clip"] = {
                "clip": widget_ref._clip_current,
                "frame": widget_ref._clip_index,
                "playing": widget_ref._clip_playing,
                "loop": widget_ref._clip_loop,
                "frame_count": len(widget_ref._succubus_clips.get(widget_ref._clip_current) or []),
                "mirror": getattr(widget_ref, "_succ_clip_mirror", False),
                "speed": getattr(widget_ref, "_speed_mult", 1.0),
                "startup_seq": getattr(widget_ref, "_startup_seq_active", False),
            }
    except Exception:
        pass

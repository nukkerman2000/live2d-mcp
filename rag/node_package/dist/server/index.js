...
import { extractSourceFromPath, generateMetaJsonPath, generateRawDataPath, isPathInRawDataDir, isPathInRawDataDirLexical, loadMetaJson, looksLikeRawDataPath, saveMetaJson, saveRawData, } from '../utils/raw-data-utils.js';
import { VectorStore } from '../vectordb/index.js';
import { DatabaseError } from '../vectordb/types.js';
import { appendConfigWarnings, buildConfigErrorBlock, formatErrorMessage, } from './error-utils.js';
...
export class RAGServer {
    ...
    constructor(config) {
        ...
        if (firstBaseDir === undefined && config.configError === undefined) {
            throw new Error('RAGServerConfig must provide either `baseDir` or a non-empty `baseDirs` array (empty `baseDirs` is allowed only in degraded mode with `configError` set).');
        }
        ...
        this.server = new Server({ name: 'rag-mcp-server', version: '1.0.0' }, { capabilities: { tools: {} } });
    ...
    setupHandlers() {
        ...
        this.server.setRequestHandler(CallToolRequestSchema, async (request) => {
            switch (request.params.name) {
                ...
                case 'ingest_data':
                    return await this.handleIngestData(request.params.arguments);
                case 'delete_file':
    ...
    async handleIngestFile(args) {
        ...
        const visualQualityArg = args.visualQuality;
        let visualQuality = 'fast';
        ...
        try {
            ...
            if (await isPathInRawDataDir(args.filePath, this.dbPath)) {
                ...
                ({ chunks, embeddings } = await buildChunksAndEmbeddings(text, title, this.chunker, this.embedder));
            }
                ...
                // (preserved below).
                const visualResult = await prepareVisualPdfChunks(args.filePath, this.parser, this.chunker, this.embedder, {
            ...
            if (chunks.length === 0) {
                throw new McpError(ErrorCode.InvalidParams, `No chunks generated from file: ${args.filePath}. The file may be empty or all content was filtered (minimum ${this.minChunkLength} characters required). Existing data has been preserved.`);
            }
     ...
     */
    async scanBaseDir(baseDir) {
        const files = [];
        const warnings = [];
        let depthLimited = false;
        const queue = [{ dirPath: baseDir, depth: 0 }];
        while (queue.length > 0) {
            ...
            for (const entry of entries) {
                ...
                if (entry.isSymbolicLink())
                    continue;
                if (this.excludePaths.some((ep) => fullPath.startsWith(ep)))
                    continue;
        ...
        if (depthLimited) {
            warnings.push(`some directories under ${displayPath(baseDir)} were skipped because they exceed the maximum depth (${RAGServer.LIST_MAX_DEPTH})`);
        }
        files.sort();
     ...
     */
    async handleReadChunkNeighbors(args) {
        try {

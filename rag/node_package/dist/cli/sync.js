...
export async function runSync(args, globalOptions = {}) {
    ...
    const stats = await executeSyncPlan(plan, diskFiles, { vectorStore, parser, chunker, embedder }, {
        onEvent: (event) => {
            switch (event.type) {
                case 'prune_start':
                ...
                case 'file_start':
                    ...
                    break;
                case 'file_ok':
                    console.error(`[${event.index}/${event.total}] ${event.filePath} ... OK (${event.chunkCount} chunks)`);
                    break;
                case 'file_empty':
                    console.error(`[${event.index}/${event.total}] ${event.filePath} ... SKIPPED (0 chunks)`);
                    break;
                case 'file_failed':
                    console.error(`[${event.index}/${event.total}] ${event.filePath} ... FAILED: ${event.error}`);
                    break;
            }
        },
    });

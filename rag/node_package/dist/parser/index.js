...
export class DocumentParser {
    ...
    async parsePdf(filePath, embedder) {
        ...
        try {
            ...
            const titleResult = extractPdfTitle(metadataTitle, firstPageChunkText, fileName, page1FontHint);
    ...
    async parsePdfPages(filePath, embedder) {
        ...
        try {
            ...
            const pages = helperPages.map((p, idx) => idx === 0 && page1FontHint !== undefined
                ? {
                    pageNum: p.pageNum,
                    text: p.text,
 ...
 */
async function extractPdfPages(doc, embedder, stextOptions) {
    ...
    const page1FontHint = titleLines.length > 0 ? { text: titleLines.join(' '), fontSize: maxFontSize } : undefined;
    const pages = pageDataList.map((p, idx) => ({
        pageNum: p.pageNum,
        text: filteredPages[idx] ?? '',
        stextJson: stextJsonList[idx],
    }));

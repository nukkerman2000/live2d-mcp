...
<label for="micLevelBar" style="min-width:120px">Уровень речи:</label>
<div id="micLevelBar" style="flex:1;height:14px;background:#333;border-radius:7px;overflow:hidden;position:relative"><div id="micLevelFill" style="width:0%;height:100%;background:linear-gradient(90deg,#22c55e,#eab308,#ef4444);border-radius:7px"></div></div>
<span id="micLevelVal" style="min-width:40px;text-align:right">0.000</span>
</div>
...
async def handle_mic_transcript(request):
    ...
    listening = stt.is_listening()
    level = stt.get_level() if listening else 0.0
    return make_response({'text': stt.get_last_transcript() if listening else '', 'listening': listening, 'level': level})

...
_stream = None
_paused = False
_audio_level = 0.0  # current RMS level for UI indicator
...
def _load_model():
    ...
    with _model_lock:
        ...
        logger.info(f"Whisper {MODEL_SIZE} loaded (cpu/int8)")
        return _model
...
def _record_and_transcribe(lang: str = "ru", device_id=None):
    ...
    def callback(indata, frames, time_info, status):
        ...
        _audio_level = float(np.sqrt(np.mean(indata[:, 0] ** 2)))
...
def stop_listening() -> str:
    global _listening, _audio_level
    _listening = False
    _audio_level = 0.0
...
def set_level(v: float):
    global _audio_level
    _audio_level = max(0.0, float(v))

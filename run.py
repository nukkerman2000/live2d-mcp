...
def main():
    ...
    def on_speech(text):
        ...
        now = time.time()
        with _speech_lock:
            if text == _last_speech["text"] and now - _last_speech["time"] < 5.0:
        ...
        try:
            ...
            if opencode_bridge.is_enabled():
                def _worker():
                    try:
                        reply = opencode_bridge.send_and_reply(text)
                        if reply:
                            import tts
                            tts.speak(reply)
                    except Exception:
        ...
        try:
            ...
            req = ur.Request(f"http://{ollama_host}:{ollama_port}/v1/chat/completions", data=payload.encode(), method="POST")
